...
export default function FiltroOrigem({ 
  ...
  const allSelected = origensSelecionadas.length === 0;

  const origensFiltradas = origensDisponiveis.filter(origem =>
    origem.label.toLowerCase().includes(busca.toLowerCase())
  );
  ...
  const getLabelTexto = () => {
    ...
    if (origensSelecionadas.length === 1) {
      const origem = origensDisponiveis.find(o => o.value === origensSelecionadas[0]);
      return origem?.label || "N/A";
    }
    return `${origensSelecionadas.length} selecionadas`;
  };
          ...
          <div className="max-h-48 overflow-y-auto p-1.5 space-y-0.5">
            {origensFiltradas.length === 0 ? (
              ...
              origensFiltradas.map((origem) => {
                const isChecked = origensSelecionadas.includes(origem.value);
                ...
                return (
                  <button
                    ...
                    onClick={() => {
                      if (isChecked) {
                        onSelectionChange(origensSelecionadas.filter(o => o !== origem.value));
                      } else {
                        onSelectionChange([...origensSelecionadas, origem.value]);
                      }
                    }}
